...
void coef_mult(register uint8_t *a, register uint8_t *b, register uint8_t *d) {
...
uint8_t i, j, col[4], res[4], a[4], tmp;

void mix_columns(uint8_t *state) {
	a[0] = 0x02;
	a[1] = 0x01;
	a[2] = 0x01;
	a[3] = 0x03; // a(x) = {02} + {01}x + {01}x2 + {03}x3
...
void sub_bytes(uint8_t *state) {
	for (i = 0; i < 4; ++i)
		for (j = 0; j < Nb; ++j) {
			tmp = (state[Nb*i+j] & 0xf0) >> 4;
			col[0] = state[Nb*i+j] & 0x0f;
			state[Nb*i+j] = s_box[16*tmp+col[0]];
		}
...
void inv_sub_bytes(uint8_t *state) {
	for (i = 0; i < 4; ++i)
		for (j = 0; j < Nb; ++j) {
			tmp = ((state[Nb*i+j] & 0xf0) >> 4);
			col[0] = state[Nb*i+j] & 0x0f;
			state[Nb*i+j] = inv_s_box[16*tmp+col[0]];
		}
...
void rot_word(register uint8_t *w) {
	tmp = w[0];
...
uint8_t* aes_keyExpansion(uint8_t *key, uint8_t *w) {
	uint8_t tmp[4];
	uint8_t i, j;
	uint8_t len = Nb*(Nr+1);
